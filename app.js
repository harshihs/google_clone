...
// Initialize Spreadsheet Data & Properties
//let spreadsheetData = Array.from({ length: rows }, () => Array(cols).fill(""));
//let cellProperties = Array.from({ length: rows }, () =>
    Array(cols).fill({
        textAlign: "left",
        bold: false,
        italic: false,
...
// ============================
// Function to Update Spreadsheet UI
// ============================
function updateSpreadsheetUI() {
    ...
    for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
            let cellName = convertRowColToCellName(row, col);
            let cellValue = spreadsheetData[row][col] || "";
            let style = cellProperties[row][col];

            // Draw cell border
            ctx.strokeRect(col * 200, row * 50, 200, 50);

            // Apply Text Formatting
            ctx.fillStyle = style.fontColor || "#000000";
            ctx.font = `${style.bold ? "bold" : ""} ${style.italic ? "italic" : ""} ${style.fontSize || "16px"} ${style.fontFamily || "Arial"}`;
            ctx.fillText(cellValue, col * 200 + 5, row * 50 + 30);

            // Underline Text
            if (style.underline) {
                ctx.beginPath();
                ctx.moveTo(col * 200 + 5, row * 50 + 35);
                ctx.lineTo(col * 200 + 5 + ctx.measureText(cellValue).width, row * 50 + 35);
                ctx.stroke();
            }
        }
    }
...
// ============================
// Click Event for Selecting Cells
// ============================
canvas.addEventListener("click", function (event) {
    const col = Math.floor(event.offsetX / 200);
    const row = Math.floor(event.offsetY / 50);
    activeCell = { row, col };
});
...
function toggleBold() {
    ...
    let props = cellProperties[activeCell.row][activeCell.col];
...
function convertCellNameToRowCol(cellName) {
    return {
        row: parseInt(cellName.slice(1)) - 1,
...
// ============================
function run_sum_func(paramString) {
    let sum = 0;
    let range = paramString.split(":");
    let start = convertCellNameToRowCol(range[0]);
    let end = convertCellNameToRowCol(range[1]);
...
function run_avg_func(paramString) {
    let sum = run_sum_func(paramString);
    let range = paramString.split(":");
    let start = convertCellNameToRowCol(range[0]);
    let end = convertCellNameToRowCol(range[1]);
    let numCells = (end.row - start.row + 1) * (end.col - start.col + 1);
    return sum / numCells;
}
...
// ============================
function run_trim_func(paramString) {
    let cell = convertCellNameToRowCol(paramString);
    return spreadsheetData[cell.row][cell.col].trim();
}

function run_upper_func(paramString) {
    let cell = convertCellNameToRowCol(paramString);
    return spreadsheetData[cell.row][cell.col].toUpperCase();
}

function run_lower_func(paramString) {
    let cell = convertCellNameToRowCol(paramString);
    return spreadsheetData[cell.row][cell.col].toLowerCase();
}

function run_find_and_replace_func(range, findText, replaceText) {
    let start = convertCellNameToRowCol(range.split(":")[0]);
    let end = convertCellNameToRowCol(range.split(":")[1]);
    let replacedCount = 0;

    for (let r = start.row; r <= end.row; r++) {
        for (let c = start.col; c <= end.col; c++) {
            if (typeof spreadsheetData[r][c] === "string" && spreadsheetData[r][c].includes(findText)) {
                spreadsheetData[r][c] = spreadsheetData[r][c].replace(new RegExp(findText, "g"), replaceText);
                replacedCount++;
            }
        }
    }
    updateSpreadsheetUI();
    return `${replacedCount} replacements made`;
}

function run_remove_duplicates_func(range) {
    let start = convertCellNameToRowCol(range.split(":")[0]);
    let end = convertCellNameToRowCol(range.split(":")[1]);
    let seen = new Set();
